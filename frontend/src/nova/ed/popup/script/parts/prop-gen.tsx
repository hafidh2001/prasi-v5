...
export const EdPropGen = () => {
  ...
  const local = useLocal({
    ...
    selected: "",
    suggest_label: "",
    suggest_status: "init" as "init" | "loading" | "ready",
  });
  ...
  if (item && item.component?.id) {
    ...
    const gen = {
      fn: undefined as undefined | ((opt: string) => string | Promise<string>),
      suggestions: undefined as
        | undefined
        | (() =>
            | Promise<{ list: string[]; label: string }>
    ...
    if (prop && prop.gen && prop.genBuilt) {
      ...
      if (gen.suggestions && local.suggest_status === "init") {
        const res = gen.suggestions();
        if (typeof res === "object" && res instanceof Promise) {
          local.suggest_status = "loading";
          res.then((val) => {
            local.suggest_status = "ready";
            local.options = val.list;
            local.suggest_label = val.label;
            local.render();
          });
        } else if (res && Array.isArray(res.list)) {
          local.suggest_status = "ready";
          local.suggest_label = res.label;
          local.options = res.list;
        }
      }
        ...
        return (
          <div className="flex border-r mr-2">
                ...
                {local.suggest_status !== "ready" ? (
                  ...
                  <label className="flex items-center border-l px-2 cursor-pointer">
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        local.selected = e.currentTarget.value;
                        local.render();
                      }}
                    >
                      <option value="" key="">
            ...
            <div
              ...
              onClick={async () => {
                if (gen.fn) {
                  const fn_result = gen.fn(local.selected);
                  let src = "";
                  if (
